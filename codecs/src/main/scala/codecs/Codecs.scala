...
trait DecoderInstances:
  ...
  given intDecoder: Decoder[Int] = Decoder.fromPartialFunction{ case Json.Num(x: BigDecimal) if x.isValidInt => x.toInt }

  given strDecoder: Decoder[String] = Decoder.fromPartialFunction{ case Json.Str(s: String) => s}

  given boolDecoder: Decoder[Boolean] = Decoder.fromPartialFunction{ case Json.Bool(b: Boolean) => b}
  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    ...
    def decodeAndPrepend(item: Json, previouslyDecodedItems: List[A]): Option[List[A]] =
      decoder.decode(item).map(json => json :: previouslyDecodedItems)
    ...
    Decoder.fromFunction {
      e => e match
        case Json.Arr(e) => decodeAllItems(e)
        case _        => None
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction{e => 
      e match
        case Json.Obj(e)  => e.get(name).flatMap( f => decoder.decode(f))
        case _            => None
    }
...
@main def run(): Unit =
  ...
  // Uncomment the following lines as you progress in the assignment
   ...
   println(renderJson(Person(null, 0)))
